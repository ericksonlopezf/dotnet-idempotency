...
flowchart TD
    ...
    subgraph Quality["Quality & Deferred Regression Gates"]
        ...
        Push_Main[Push to main] --> Stryker[Mutation Testing Matrix<br/>13 Packages Async Gate]
        Cron_Sun[Weekly Cron Schedule] --> Stryker
...
Performs comprehensive mutation testing using Stryker.NET as an asynchronous deferred quality gate for `main` and releases.

- **File**: `.github/workflows/mutation-testing.yml`
- **Triggers**:
  - `push` to `main` (ignoring `**.md`, `docs/**`)
