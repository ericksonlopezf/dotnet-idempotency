...
| **Sensitive Header Leakage** | Storing `Authorization`, `Cookie`, or `Set-Cookie` headers in cached responses. | Configurable `ResponseHeadersBlocklist` strips sensitive headers (`Set-Cookie`, `Set-Cookie2`, `Authorization`, `Proxy-Authenticate`, `Proxy-Authorization`, `WWW-Authenticate`) before persisting. |
...
- Use field redaction or custom serializers (`IIdempotencySerializer`).
- `ResponseHeadersBlocklist` prevents credential and session cookie persistence.
- Disable body caching for sensitive endpoints and rely on status-code-only replay (`CacheOnlySuccessResponses = true`).
