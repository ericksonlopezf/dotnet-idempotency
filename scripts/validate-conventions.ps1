...
<#
.SYNOPSIS
    Automated Convention and Quality Gate Validator for EricksonLopez.Idempotency.
.DESCRIPTION
...
$mdFiles = Get-ChildItem -Path @("docs", ".github", ".") -Filter "*.md" -Recurse -File | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|\.git|\.vs|\.system_generated|StrykerOutput|MEGA-AUDITORIA|node_modules|TestResults|CoverageReport|artifacts|benchmarks|BenchmarkDotNet\.Artifacts)[\\/]'
}
...
foreach ($file in $allScanFiles) {
    ...
    if ($content -match 'github\.com/ericksonlopez/dotnet-idempotency') {
        $rel = Resolve-Path -Relative -Path $file.FullName
        $violations.Add("Legacy GitHub repository URL found in: $rel (expected: github.com/ericksonlopezf/dotnet-idempotency)")
        $legacyUrlCount++
    }
