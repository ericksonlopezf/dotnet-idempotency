...
Write-Host "`n[3/7] Checking canonical MIT copyright headers across all source files..." -ForegroundColor Yellow
$scanDirs = @("src", "tests", "samples", "benchmarks") | ForEach-Object { Join-Path $RootDirectory $_ }
$allCsFiles = Get-ChildItem -Path $scanDirs -Recurse -Filter "*.cs" -File | Where-Object { $_.FullName -notmatch '[\\/](obj|bin)[\\/]' }
$missingHeaders = 0
...
if ($missingHeaders -eq 0) { Write-Host "  ✅ All C# source files ($($allCsFiles.Count)) contain the required MIT copyright header." -ForegroundColor Green }
...
if (-not (Test-Path $bldPropsPath)) {
    ...
    if ($bldContent -notmatch '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Directory.Build.props must specify '<ImplicitUsings>disable</ImplicitUsings>'." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Directory.Build.props missing ImplicitUsings disable") } else { $Violations++ } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $sourceLinkErrors++
    }
    if ($bldContent -notmatch '<PackageProjectUrl>\s*https://ericksonlopez\.dev/idempotency\s*</PackageProjectUrl>') {
        Write-Host "  ❌ Directory.Build.props must specify '<PackageProjectUrl>https://ericksonlopez.dev/idempotency</PackageProjectUrl>'." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Directory.Build.props invalid PackageProjectUrl") } else { $Violations++ } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $sourceLinkErrors++
    }
}
...
if ($sourceLinkErrors -eq 0) {
    Write-Host "  ✅ SourceLink, ImplicitUsings, and PackageProjectUrl verified in Directory.Build.props." -ForegroundColor Green
}
